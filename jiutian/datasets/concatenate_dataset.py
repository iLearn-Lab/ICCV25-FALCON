# This file is adopted from https://github.com/shikras/shikra
...
class ConcatDataset(Dataset):
    ...
    def __init__(self, datasets: List[Dataset]):
        ...
        lengths = [getattr(ds, "lengths", None) for ds in datasets]
        if all(l is not None for l in lengths):
            self.lengths = [l for ds_lengths in lengths for l in ds_lengths]

        modality_lengths = [getattr(ds, "modality_lengths", None) for ds in datasets]
        if all(l is not None for l in modality_lengths):
            self.modality_lengths = [l for ds_lengths in modality_lengths for l in ds_lengths]
...
# borrowed from huggingface/datasets
...
class SubSet(TorchSubset):
    def __init__(self, dataset, portion, do_shuffle=True, seed=42):
        assert 0 < portion <= 1
...
class ConcatDatasetWithShuffle(TorchSubset):
    ...
    def __init__(self, datasets, seed=42, portion=1):
        self.seed = seed
