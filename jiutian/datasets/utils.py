...
from jiutian.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN
...
def preprocess_llama3(
    ...
    date_str = '<|start_header_id|>system<|end_header_id|>\n\nCutting Knowledge Date: December 2023\nToday Date: 26 Jul 2024\n\n'
    date_idx = tokenizer.encode(date_str)
    ...
    for i, source in enumerate(sources):
        ...
        for conv in source:
            ...
            # FIXME: REPEATED DATE
            if len(date_idx) > 0:
                encode_id = encode_id[len(date_idx):]
...
def preprocess(
    ...
    if conversation_lib.default_conversation.version == "qwen":
        return preprocess_qwen(
            sources, tokenizer, has_image=has_image, system_message=conversation_lib.default_conversation.system
        )
